...
const Cart = () => {
  ...
  useEffect(() => {
    try {
      ...
      const productIds = stored ? JSON.parse(stored) : null;
      if (Array.isArray(productIds) && productIds.length > 0) fetchCartProducts(productIds);
    } catch {
  ...
  const buyProducts = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        alert("You need to log in before purchasing.");
      } else {
